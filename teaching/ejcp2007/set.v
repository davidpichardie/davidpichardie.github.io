...
(* Un exemple d'implementation avec les entiers de Peano *)
...
Module Type FSetDep.
  ...
  Parameter elt : Set.
  Parameter t : Set.
  Parameter In_set : elt -> t -> Prop.
  Parameter empty : { s:t | forall x, ~ In_set x s }.
  Parameter mem : forall (x:elt) (s:t), { b:bool | b = true <-> In_set x s}.
  Parameter elements : forall s:t, { l:list elt | forall x, In x l <-> In_set x s}. 
  Parameter add : forall (a:elt) (s:t), { s':t | forall x, In_set x s' <-> (x=a \/ In_set x s)}.
  Parameter remove : forall (a:elt) (s:t), { s':t | forall x, In_set x s' <-> (x<>a \/ In_set x s)}.
  Parameter union : forall (s1 s2:t), { s:t | forall x, In_set x s <-> (In_set x s1 \/ In_set x s2)}.
  Parameter inter : forall (s1 s2:t), { s:t | forall x, In_set x s <-> (In_set x s1 /\ In_set x s2)}.

End FSetDep.

(* Exercice : faire un foncteur 
Module BridgeNotDepToDEp (F:FSet) : FSetDep 
  with Definition t:=F.t
  with Definition elt:=F.elt.
*)
...
Module Make (O:OrderedType) : FSet with Definition elt := O.t.
  ...
  Proof.
    induction l; simpl.
    discriminate.
    case_eq (O.compare a x); intros; auto.
    discriminate.
  Qed.
  ...
  Proof.
    ...
    case_eq (O.compare a x); auto; intros.
    assert (HH:=H x H1).
    unfold O.lt in HH.
    rewrite H2 in HH.
    discriminate.
