...
(** Our first Coq program *)

(** In this tutorial we will implement simple maps *)
Module Type MAP.
...
(** Observe that we use the same keyword for types and operators.
    What we think as types here, are terms of type [Type]. *)
...
Module Type MAP.
  ...
  Parameter empty: t.
  Parameter empty_spec: forall (k:key), (* TODO *)
...
Module NatStringMap <: MAP 
 ...
 Fixpoint set (l:list string) (n:nat) (s:string) : list string :=
  ...
  (** This time we can't end the module so easily because we have proofs
   to do ! *)
  ...
  (** The command we use to build an interactive proof are called tactics.
    ...
    a custom tactic that forces Coq to search the proof on its own *)
...
Check xI (xI (xO xH)).
(** represents 1 + 2 * (1 + 2 *  (0 + 2 * 1)) = 11 = Ox1011 *)
...
(** A custom tactic to invert an inductive hypothesis *)
...
Definition bin_10_4 : bin 4 := Bin 4 (xO (xI (xO xH))) inf_log_10_4.
...
Definition bin_10_5 : bin 5 := Bin 5 (xO (xI (xO xH))) inf_log_10_5.


(** We want now to define the successor of a binary number *)
...
(** Some auxiliaries lemmas are required *)
